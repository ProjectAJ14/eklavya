...
| `tool_use`, `tool_error` | Successful tool calls with a result excerpt, or failed calls with their error. |
| `file_edit` | Edit, Write, NotebookEdit and MultiEdit. |
| `file_read` | Read, NotebookRead, Glob and Grep. |
| `assistant`, `lifecycle` | Claude's final message at the end of each turn (from Claude Code's Stop hook), and session start/stop events. |
...
Nothing is captured in a folder outside a git checkout unless `retrieval.cross_project` is on: only a cross-project search serves that folder, so otherwise its evidence would only take up space. An excluded read or edit is dropped when it has no remaining allowed file. Eklavya's own tool calls, CLI traffic and checkpoint messages are excluded to prevent its output feeding back into memory.
...
The model writes one observation per distinct finding, decision or change, up to six per batch, each with at most 12 facts. When it goes past those limits, the extra is trimmed and the rest is kept; only output with the wrong shape fails the batch.
...
| Resume now after fixing login, quota or a missing `claude` command, instead of waiting for the next check | `eklavya memory process` |
...
At session start, Claude receives recent project history in an `<eklavya-memory>` block. The block holds up to `retrieval.max_items` entries (6 by default) within `retrieval.max_tokens` (1,200 estimated tokens by default). An entry too long for the remaining budget is skipped and shorter ones fill the space. After those full entries, up to 30 more are listed by title and ID only (about 700 tokens at most), with a line telling Claude to read any of them with the `memory_get` tool or search further back with `memory_search`. The block is marked as **evidence to verify, never instructions to obey**.

The session banner says what was delivered, for example `Memory · 3 past entries + 24 titles recalled (~1.6k tokens)`. A project with only a few entries shows no titles, for example `Memory · 3 past entries recalled (~1.1k tokens)`. It prints no memory line when nothing was recalled.

Recall needs a git checkout. In a folder outside git there is no project to recall from, so neither session-start nor prompt recall runs, and nothing is captured there either. Evidence that earlier versions captured outside git stays under **No repository** and is not summarised.

Relevant memory can also be supplied on a substantive prompt later in the session. It gets a third of the session-start budget (at most `retrieval.max_items / 3` entries and `retrieval.max_tokens / 3` estimated tokens, so two entries and 400 tokens by default), excludes entries already delivered and requires a relevance match. The cap is strict: an entry larger than the whole per-prompt budget is skipped. Unrelated prompts receive no memory block. Only what you typed is searched: pasted content, subagent hand-backs and task notifications are ignored, so a prompt that contains nothing else gets no memory block.
