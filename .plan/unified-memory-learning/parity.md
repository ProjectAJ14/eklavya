...
| PAR-03 | partial | Transcript replay is done: `memory/replay.ts` reads the transcript directory, applies the privacy filter, is idempotent, and converges with hook capture by content (`memory-replay.test.ts`, eleven cases). **Session summaries were not.** An audit of this ledger against the tree found that nothing generates one -- `memory_entries.kind` defaults to `'observation'`, `worker.ts` never passes a kind, and the only writer of `kind='session_summary'` was the importer. An imported Claude Mem history therefore had summaries and a natively captured one never would. In progress. |
