...
Every concept can be asked about at five depths. The tier is chosen per question. It is not printed with the question — a [status bar](/docs/cli/#eklavya-statusline) refreshes when Claude Code decides to, not when a question appears, so a tier shown there would sometimes name the previous one. What the bar does carry is your **level**, which is the part that explains the pitch: `easy` means tiers 1–2. The exact tier of any answer is in [the dashboard](/docs/dashboard/).
...
A project is the git checkout you are in, resolved to its main one: a [git worktree](https://git-scm.com/docs/git-worktree) is a branch of the same codebase, so its answers, level and progress all count towards the repository it was created from rather than starting a fresh `easy` runway per branch. Two separate clones of the same repository are still two projects — they are two places on disk with nothing linking them.

### Pinning a level

```text
eklavya config set difficulty hard          # skip the runway, for you
eklavya config set difficulty easy --project  # keep one codebase gentle for you
```
