...
| **The hooks** | `hooks/` and `mcp/src/hooks/` — small Node scripts | The timing. They decide *when* a question arrives: at session start, on a prompt in a session that has gone quiet, mid-task, at the end of a task, and before a `git commit`. |
