...
- It asks which scope you mean when it is ambiguous: **global** (you, everywhere), **repo** (this project, for everyone) or **session** (this session only, and only for `off`).
- **"Turn Eklavya off for this session"** — in those words, or `--session` — silences everything until the session ends without writing to either config file. It does not open the commit gate: an `enforced` repo still holds the commit. See [Configuration](/docs/configuration/#turning-it-off-for-one-session).
