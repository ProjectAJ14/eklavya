...
| `src/install-lock.ts` | the one lock on `~/.eklavya/runtime`: the `.installing` stamp `installRuntime`, the updater and `hooks/run.mjs` all take — an exclusive create, an owner pid plus a token, stale claims broken by rename-and-verify, release only by token. `run.mjs` carries a copy of the rules | never an OS lock: its two known gaps are written at the top |
...
| `src/onboard.ts` | the settings walk `install` runs on a first install (no `~/.eklavya/config.json`) or with `--settings` — one step per dial, an arrow-key list starting on the current value (`press` is the pure key reducer); writes only the global config, only what changed — plus an empty write after a first walk, since the file existing is what "chosen" means | never asks about project scope; unasked, never decides the Claude Mem question on a re-install |
| `src/update.ts` | the auto-updater: `startBackgroundUpdate` (SessionStart, at most hourly, detached), `runUpdate` (`npm view` → `npm install` into `~/.eklavya/runtime` → the new runtime's `install --auto`), `updateNotice` (the one session line), and `update.json`/`update.log` beside the database. Shares the runtime lock (`install-lock.ts`) with `run.mjs` and `installRuntime`. `cli.ts` hands off to the runtime when it is newer, so the global binary is never stale | never downgrades, never touches an `EKLAVYA_RUNTIME` build, never needs sudo; light imports, because SessionStart loads it every session |
...
| `src/artifacts.ts` + `src/assets/artifact-template.html` | `eklavya artifacts new/list/open`: a page's path (`~/.eklavya/artifacts/<projectSlug>/<local date>-<kebab>.html`, a worktree folded to its main checkout), its metadata `<meta>` tags, and the template with `tokens.css` inlined at creation — one palette, no second copy. `listArtifacts` reads the files' heads; there is no table. `resolveArtifact` is the dashboard's only way to a file: `<folder>/<file>.html`, no dot-names, realpath inside the root | never overwrites a page; never publishes anything |
...
`LATEST_SCHEMA_VERSION` (currently 15), the file list in the "upgrades an older
