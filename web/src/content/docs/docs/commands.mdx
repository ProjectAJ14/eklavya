...
Seven slash commands, all typed inside Claude Code. Every one of them is read-only until you ask for a change, and none of them starts a quiz unless that is what the command is for. If you would rather not memorise them, [you can just ask](/docs/commands/#or-just-ask).
...
- Questions come one at a time, each at its own tier, each graded before the next.
...
The value tells Eklavya which dial you mean, so you never have to name the dial. Values are unambiguous across the four.
...
- How many concepts you have answered out of the number required.
- The `pass_threshold` those answers have to clear.
- Whether the gate has passed.

If it has not passed, it says exactly what remains and offers to run the quiz — it will not start one without being asked.
