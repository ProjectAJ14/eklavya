...
:::note
**git is optional.** Without it Eklavya still runs, but two things change: your difficulty level is tracked in one shared bucket instead of per project (worktrees of one repository count as one project), and the commit gate has nothing to attach to. Everything else works.
:::
