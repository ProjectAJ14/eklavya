...
describe('concurrent migrations', () => {
  it('every process survives an upgrade that adds columns, and each migration runs exactly once', async () => {
    ...
    expect(applied).toEqual(['013_batch_provenance.sql', '014_batch_events_index.sql', '015_event_link_indexes.sql', '016_usage_counts.sql']);
  ...
  it('waits out a writer that holds the lock longer than the caller\'s busy_timeout', async () => {
    ...
    try {
      expect(runMigrations(db)).toEqual(['013_batch_provenance.sql', '014_batch_events_index.sql', '015_event_link_indexes.sql', '016_usage_counts.sql']);
