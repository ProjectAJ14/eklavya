...
| `commands.mdx` | one `###` per slash command — the skills under `skills/` carrying `disable-model-invocation: true`, nine of them; `skills/tutor/` is model-invocable only and gets no heading. Plus one for each skill under `user-skill/` — `eklavya` and `eklavya-artifacts` |
