...
export const getSessionQuizPlan: ToolDef = {
  ...
  handler: (
    ...
    if (!config.quiz.enforced && !args.ignore_cooldown) {
      ...
      if (since < gap) {
        return {
          ...
          concepts: [],
          reason: 'cooldown',
          minutes_remaining: Math.ceil(gap - since),
        };
      }
    }
    ...
    if (topicMode) {
      ...
      //
      // Scoped to this session's domains when it has any. `session_concepts` has
      // no repo column and there is one database for every project, so without
      // this a Flutter session gets asked about the Terraform it logged last
      // month -- while `framing` is still telling the tutor to ground the
      // question in this session's diff, which the debt is not in. The unscoped
      // fallback is for a session that logged nothing at all: there is no diff to
      // contradict, and offering the debt beats saying there is nothing to ask.
      //
      ...
      if (picked.length < max && !config.quiz.enforced) {
        ...
        const rows = db
          ...
          .all(sessionId, ...(scoped ? [...domains] : [])) as ConceptRow[];
    ...
    return {
      ...
      // Returned with every plan for the same reason `framing` is: this is the
      // one place every ask path passes through -- both hooks, the quiz
      // command and the tutor subagent -- and the rule depends on a host only
      ...
      // The Stop sweep used to say this, but a Stop hook's text is printed to
      // the developer verbatim, and "if they say skip, let them go" is an
      // instruction about them, not to them. Here only the model reads it.
      on_skip: config.quiz.enforced
        ? isCowork()
