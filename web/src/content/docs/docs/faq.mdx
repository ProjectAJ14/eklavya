...
No. The database, the config and the dashboard are all local, and the dashboard binds to loopback. There is no account and no telemetry. The only network access in normal use is npm fetching the server package when you install or upgrade.
...
Run `/plugin` and update Eklavya from the menu, then restart Claude Code — hooks and MCP servers are wired up when a session starts, so the running one keeps the old build.
