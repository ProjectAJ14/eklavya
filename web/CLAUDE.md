...
|---|---|
| `index.mdx` | the loop as it actually runs: the seven hooks in `mcp/src/hooks/` and `max_questions_per_task` in `mcp/src/config.ts`. It also carries the one prerequisite — `mcp/package.json` `engines` — because `before-you-start.mdx` was merged into it. Keep it short: it is the page that says what the tool is, not how it works |
| `installing.mdx` | `mcp/src/install.ts` (its numbered steps are the numbered steps on the page), `mcp/src/safe-write.ts` (the `.eklavya-bak` backups and the refusal to touch an unparseable file), `.mcp.json`, `hooks/run.mjs` (the background runtime heal). `README.md` points here rather than repeating it — keep it that way |
...
| `commands.mdx` | one `###` per slash command — the skills under `skills/` carrying `disable-model-invocation: true`, nine of them; `skills/tutor/` is model-invocable only and gets no heading. Plus one for `user-skill/` |
