...
`agents/tutor.md` defines an `eklavya-tutor` subagent with read-only file access (`Read`, `Grep`, `Glob`), six of the nine learning MCP tools — `get_learner_profile`, `get_session_quiz_plan`, `record_attempt`, `get_concept_graph`, `upsert_concepts`, `get_gate_status` — and three read-only memory tools: `memory_search`, `memory_get`, `memory_file_history`. Not `log_session_concepts`, and not `get_config`/`set_config` — the tutor teaches what the builder logged; it does not log work of its own or change the dials. Not `memory_write`, `memory_correct` or `memory_delete` either, for the same reason: a tutor that edits the project's record of itself is a tutor that can rewrite the evidence its next question is graded against. Each name is listed twice, plugin-scoped and bare, so one set resolves whichever way the server was registered. Ask for it by name:
...
- **Both panes capture.** `capture-tool` has no session guard, so each pane records its own tool calls, and with a shared `EKLAVYA_SESSION_ID` both write into one session's evidence — which is what makes the teaching pane's `memory_search` able to see what the building pane just did. `event_uid` is a hash over host, session, agent, kind, tool, timestamp and body (`mcp/src/memory/identity.ts`), so the teaching pane reading a file the building pane also read is two events, not a collision. Whichever pane's Stop hook fires first closes the batch; the other finds nothing pending and returns.
