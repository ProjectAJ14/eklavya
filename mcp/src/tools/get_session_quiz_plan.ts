...
import { minutesSince, parseStamp } from '../time.js';
import { isSessionOff, resolveSessionId, workSince } from '../session.js';
import {
  attemptedConceptIds,
  domainSiblings,
  gateRetryConcepts,
...
export const getSessionQuizPlan: ToolDef = {
  ...
  handler: (
    ...
    // It also outranks `learn` focus: a topic answer is recorded as review, the
    // gate counts only work, so a standing topic would fill every slot with
    // questions that cannot clear the commit and leave it blocked for good.
    // An explicit domain or slugs still win -- that is the developer asking.
    const openGate = config.quiz.enforced && !explicitTopic ? gateRow(db, sessionId) : undefined;
    const gateOpen = Boolean(openGate && openGate.required > 0 && !openGate.passed);

    if (!explicitTopic && focus === 'learn' && !gateOpen) {
    ...
    if (topicMode) {
      ...
      // What (a), (b) and widening may use: work logged in the current stretch
      // (`workSince`), the boundary both quiz hooks apply, so the concept a hook
      // names is the one this plan serves. Seconds, as SQLite's datetime()
      ...
      if (focus === 'concept' && picked.length < max && !gateOpen) {
        // Widened from the current stretch's work, so an overnight session is
        // not asked about the prerequisites of yesterday's code.
        const exclude = new Set(touched.map((c) => c.id));

        for (const c of prereqsOf(db, current.map((c) => c.id))) {
          const m = masteryFor(db, c.id);
        ...
        for (const c of domainSiblings(db, [...new Set(current.map((c) => c.domain))], exclude)) {
      ...
      // or missed, due again -- the only backlog there is (`dueInProject`).
      // Last, so it never displaces the session the developer is actually in. Read through this checkout: one database
    ...
    if (picked.length === 0) {
      ...
      //
      // A session quiz is scoped to this project, so an empty one also says
      // where the work is waiting -- otherwise "nothing to ask" reads as
      // "nothing anywhere". Topic mode already spans every project.
      const elsewhere = topicMode ? [] : pendingElsewhere(db, repoRoot, now);
      const extra = elsewhere.length > 0 ? { pending_elsewhere: elsewhere } : {};
