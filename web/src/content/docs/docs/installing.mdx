...
1. #### Checks your Node version
   ...
   Stops with upgrade instructions if it is older than 22, rather than half-installing and failing later.
...
3. #### Installs and enables the plugin
   ...
   Copies the plugin into Claude Code's plugin directory and registers it, which is the same end state `/plugin install` produces.
...
Then restart Claude Code and run `/eklavya:setup` to choose a mode. Running `npx eklavya install` again is how you upgrade — every step is safe to repeat, and it will not disturb your other plugins or your learning history.
...
2. #### Install the plugin
   ...
   The part before the `@` is the plugin, the part after is the marketplace it comes from — they happen to share a name. Claude Code downloads the plugin and registers its MCP server, its four hooks and its seven slash commands.
...
[Eklavya] No learning history yet. Mode: ambient. Focus: concept. Cadence: interleaved. Level: easy.
```
...
### Getting the `eklavya` command

Some things — checking your setup, serving the dashboard, editing config from a terminal — are done with a small CLI rather than a slash command. `npx eklavya install` already put it on your machine; to have it permanently on your `PATH`:
