---
title: "First run"
description: "Set up Eklavya for your first project. Choose learning and memory settings, build your learner profile, and start with questions matched to your work."
sidebar:
  order: 3
---
...
3. #### It asks you to choose a mode
   ...
   - **questions, unenforced** (default) — they arrive as you work and are always skippable.
   - **enforced** — you must pass the session quiz before a commit goes through. For interns and onboarding.
   - **no questions** — `quiz.enabled: false`. This stops the questions only; memory keeps recording, and `memory.enabled` is the separate switch for that.
...
4. #### It asks you to choose a focus
   ...
   Both answers are written to `~/.eklavya/config.json`, your global settings. Setup does not ask about per-project settings. If you want one project to differ, say so, or run `eklavya config set <key> <value> --project`. That writes `~/.eklavya/projects/<checkout>/config.json`, still in your home directory. Eklavya writes nothing into the repository. You can change them any time with [`/eklavya:mode`](/docs/commands/#eklavyamode-value).
5. #### If you chose enforced, it installs the git hook
   ...
   Without this, only commits made inside Claude Code are gated. See [the commit gate](/docs/commit-gate/). Skipped unless you chose enforcement.
